...
public class put_common_method {
	...
	public static int statuscodeextractor(String baseURI, String resource, String requestbody) {
		...
		int status_code = given().header("Content-Type", "application/json").body(requestbody).when().put(resource)
				.then().extract().statusCode();
		return status_code;
	}

	public static String responsebodyextractor(String baseURI, String resource, String requestbody) {
		String res_body = given().header("Content-Type", "application/json").body(requestbody).when().put(resource)
